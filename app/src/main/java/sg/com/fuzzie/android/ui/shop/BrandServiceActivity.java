...
public class BrandServiceActivity extends BaseActivity {
    ...
    @ViewById(R.id.llPrices)
    LinearLayout llPrices;

    @ViewById(R.id.tvPrice)
    TextView tvPrice;

    @ViewById(R.id.tvStrikePrice)
    ...
    public void calledAfterViewInjection() {
        ...
        service = Service.fromJSON(serviceExtra);
        ...
        setBuyAndGiftIt(service.isSoldOut());
        ...
        tvStrikePrice.setPaintFlags(tvStrikePrice.getPaintFlags() | Paint.STRIKE_THRU_TEXT_FLAG);
        if(service.getDiscountedPrice() > 0) {

            tvPrice.setText(service.getPrice().getCurrencySymbol() + String.format("%.0f", service.getDiscountedPrice()) );

            if (service.getDiscountedPrice() == Float.valueOf(service.getPrice().getValue())) {
                tvStrikePrice.setText("");
            } else {
                tvStrikePrice.setText(service.getPrice().getCurrencySymbol() + service.getPrice().getValue() );
            }

        } else {
            tvPrice.setText(service.getPrice().getCurrencySymbol() + service.getPrice().getValue() );
            tvStrikePrice.setText("");
        }
        ...
        tvBrandName.setText(service.getDisplayName());
        ...
        rvGiftCards.setVisibility(View.GONE);
    }
    ...
    void buyButtonClicked() {
        ...
        dataManager.selectedService = service;
    ...
    void termsButtonClicked() {
        ...
        if (service != null && service.getTermsAndConditionsList() != null && service.getTermsAndConditionsList().size() > 0){

            StringBuilder sb = new StringBuilder();
            for (String terms: service.getTermsAndConditionsList()) {
                sb.append(terms);
                sb.append("\n");
            }
    ...
    private void initCashbackInfo(){
        ...
        if (service.getCashBack().getPercentage() > 0) {

            if (service.getCashBack().getPowerUpPercentage() > 0) {

                tvPowerUp.setText(FuzzieUtils.getFormattedPowerUpPercentage(service.getCashBack().getPowerUpPercentage(), 1));
                ...
                if (currentUser.getCurrentUser().getPowerUpExpirationTime() != null || brand.isPowerUp()) {
                    ...
                    tvPowerUpPercentage.setText(String.format("Get +%s%% Cashback", FuzzieUtils.getFormattedPercentage(service.getCashBack().getPowerUpPercentage())));
            ...
            if (service.getCashBack().getCashBackPercentage() > 0) {

                tvCashback.setText(FuzzieUtils.getFormattedCashbackPercentage(service.getCashBack().getCashBackPercentage(), 1));
            ...
            if (currentUser.getCurrentUser().getPowerUpExpirationTime() != null || brand.isPowerUp()) {
                ...
                tvCashbackPercentage.setText(String.format("%s%% Instant Cashback", FuzzieUtils.getFormattedPercentage(service.getCashBack().getPercentage())));
                double cashback = Double.parseDouble(service.getPrice().getValue()) * service.getCashBack().getPercentage() / 100;
                tvEarn.setText(FuzzieUtils.getFormattedValue(cashback, 0.75f));

            } else {

                tvCashbackPercentage.setText(String.format("%s%% Instant Cashback", FuzzieUtils.getFormattedPercentage(service.getCashBack().getCashBackPercentage())));
                tvEarn.setText(FuzzieUtils.getFormattedValue(service.getCashBack().getValue(), 0.75f));
            }
    ...
    private void showRedeemValidInfo(){
        ...
        try {
            ...
            Date endDate = dateFormat.parse(service.getRedeemEndDate());
        ...
        call.enqueue(new Callback<List<String>>() {
            ...
            public void onResponse(Call<List<String>> call, Response<List<String>> response) {
                if (response.code() == 200 && response.body() != null){
                    ...
                    logoutUser(currentUser, dataManager,  BrandServiceActivity.this);
                }
            }
                ...
                if (response.code() == 200 && response.body() != null && response.body().getReviewsCount() != null && response.body().getReviewsCount() != 0 && response.body().getLink() != null && !response.body().getLink().equals("")){
                    ...
                    btnTripAdvisor.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View v) {
                            ...
                            customTabsIntent.launchUrl(BrandServiceActivity.this, Uri.parse(response.body().getLink()));
                        }
                    });
...
//                            Timber.d(TAG, "File Created: " + created);
                        ...
                        String text = String.format("%s: %s: ", brand.getName(), service.getName());
                        if (service.getDiscountedPrice() > 0) {
                            text +=  String.format("%s%s (Usual %s%s) ", service.getPrice().getCurrencySymbol(), service.getDiscountedPrice(), service.getPrice().getCurrencySymbol(), service.getPrice().getValue());
                        } else {
                            text +=  String.format("%s%s ", service.getPrice().getCurrencySymbol(), service.getPrice().getValue());
                        }
                        if (service.getCashBack().getPercentage() > 0) {
                            if (service.getCashBack().getPowerUpPercentage() > 0) {
                                text += String.format("+ %.0f%% INSTANT Cashback + %.0f%% Bonus Cashback. Get it on FUZZIE now: http://fuzzie.com.sg/", service.getCashBack().getPercentage(), service.getCashBack().getPowerUpPercentage());
                            } else {
                                text += String.format("+ %.0f%% INSTANT Cashback. Get it on FUZZIE now: http://fuzzie.com.sg/", service.getCashBack().getPercentage());
                            }
                        }
